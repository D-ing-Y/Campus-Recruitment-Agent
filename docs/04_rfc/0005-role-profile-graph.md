...
因此岗位画像不能只是“调用爬虫后让 LLM 总结”。Graph 必须保存检索意图、查询和来源运行记录，保证 raw-before-parse，分离来源权威，显式统计样本与分母，并在预算内自主决定继续搜索、换词、换源、等待授权或停止。

## 2. 目标

- 复用 v0.3 Evidence Store 与 v0.4 LangGraph/checkpoint 模式。
- 实现 recruitment/experience 两类 SourceAdapter 和统一运行外壳。
- 真实接入一个招聘来源和一个经验来源，默认 CI 保持离线。
...
| --- | --- | --- | --- |
| `zhaopin_jobs` | recruitment | `https://xiaoyuan.zhaopin.com/` | 校招岗位发现与岗位详情 |
| `nowcoder_experience` | experience | `https://www.nowcoder.com/` | 校招笔经、面经和招聘过程信号 |

选择原则：

- 招聘平台用于未知公司范围的首版发现；企业官网只在指定公司或已有岗位 URL 时验证。
...
### 7.6 `extract_and_validate_role_claims`
...
4. 当前 query 可继续分页 → `search_more`；
5. relevance/recall 缺口可通过同义词改善 → `change_query`；
6. authority/channel/source failure 缺口 → `change_source`；
7. 达到覆盖标准 → `complete`；
8. 边际价值低或无可用来源 → `finalize_with_unknowns`。
...
```python
class RecruitmentSourceAdapter(Protocol):
    source_id: str
...
- recruitment/experience channel 分离；
- pagination/search_more、change_query、change_source；
...
- `zhaopin_jobs`：显式启用一次小范围查询，保存 raw 和 SourceRunReceipt。
