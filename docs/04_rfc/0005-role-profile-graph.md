...
| --- | --- | --- | --- |
| `zhaopin_jobs` | recruitment_discovery | `https://www.zhaopin.com/` | 核心第三方岗位发现与详情入口 |
...
- `zhaopin_jobs`：显式启用一次小范围查询，保存 raw 和 SourceRunReceipt；若需要登录，仅接受正常 Chrome 登录态的本地 credential ref。
- `official_careers`：从一个智联候选岗位出发，在企业官网完成一次身份链接和字段核验。
